...
- [x] T001 Review existing project structure against plan.md for completeness
- [x] T002 [P] Update SpacetimeDB SDK dependencies to 2.0.1+ in spacetimedb/package.json
- [x] T003 [P] Verify React 18.3.1 and Vite 7.1.5 versions in package.json
- [x] T004 [P] Review constitution.md principles and ensure project follows all 6 principles
...
- [x] T005 Add StreamerProfile table to spacetimedb/src/index.ts (id=ADMIN_IDENTITY PK, name, bio, avatarUrl, socialLinks as JSON string, streamStatus 'online'/'offline')
- [x] T006 [P] Add StreamScheduleDay table to spacetimedb/src/index.ts (dayNumber 1-7 PK, theme, description)
- [x] T007 [P] Add ReportedMessage table to spacetimedb/src/index.ts (id auto-inc PK, messageId, reporterIdentity, reportedAt timestamp, status: 'pending'/'reviewed'/'resolved'; admin-only access)
- [x] T008 [P] Add ChannelUnread table to spacetimedb/src/index.ts (userId+channelId composite key, lastReadTimestamp)
- [x] T009 Add avatarUrl field to User table in spacetimedb/src/index.ts (optional string)
- [x] T010 [P] Add isLiveChat field to Channel table in spacetimedb/src/index.ts (boolean)
- [x] T011 Add character limit check (2000 max) to send_message reducer in spacetimedb/src/index.ts (FR-B08)
- [x] T012 [P] Create scheduled reducer cleanup_old_sessions in spacetimedb/src/index.ts (7-day retention per FR-F08)
- [x] T013 Publish updated module: `spacetime publish` + `spacetime generate`

### Frontend Infrastructure

- [x] T014 Create src/types/streamer.ts for StreamerProfile, StreamScheduleDay, ReportedMessage types
- [x] T015 [P] Create src/utils/constants.ts for MAX_MESSAGE_LENGTH=2000, OFFLINE_FILTER_DAYS=7, ADMIN_IDENTITY constant, and 7 default schedule themes (Stardew Valley, Farming Games, Fantasy Adventure, Science Fiction, Horror/Scary, Puzzle/Platformer, Any Category)
- [x] T016 [P] Add Rosie the Riveter SVG mascot asset to src/assets/rosie-raven.svg
- [x] T017 [P] Create src/hooks/useStreamStatus.ts for stream online/offline state management
- [x] T018 [P] Create src/hooks/useChannelUnread.ts for unread indicator state management
...
- [x] T028 [B1] Verify send_message reducer handles channel_id in spacetimedb/src/index.ts (FR-B01)
- [x] T029 [B1] Confirm message subscription in src/hooks/useChatMessages.ts uses useTable(tables.message) (FR-B03)
- [x] T030 [B1] Ensure MessageList in src/components/Chat/MessageList.tsx displays chronologically (FR-B04)
- [x] T031 [B1] Add message validation UI feedback in src/components/Chat/MessageInput.tsx for 2000 char limit (FR-B09)
...
- [x] T032 [B2] Implement scroll position detection in src/components/Chat/MessageList.tsx (FR-B05, FR-B06)
- [x] T033 [B2] Add auto-scroll logic on new message arrival in MessageList.tsx (FR-B05)
- [x] T034 [B2] Implement pause auto-scroll when user scrolls up in MessageList.tsx (FR-B06)
- [x] T035 [B2] Resume auto-scroll when user returns to bottom in MessageList.tsx (FR-B07)
...
- [x] T036 [C1] Verify toggle_like reducer implements self-like prevention in spacetimedb/src/index.ts (FR-C04)
- [x] T037 [C1] Add like count display to src/components/Chat/MessageItem.tsx (FR-C03)
- [x] T038 [C1] Implement like/unlike button toggle in MessageItem.tsx (FR-C02)
- [x] T039 [C1] Add toast notification for self-like attempt in MessageItem.tsx (FR-C04)
- [x] T040 [C1] Respect ENABLE_MESSAGE_LIKES flag in src/config/featureFlags.ts (FR-C06)
...
- [x] T041 [D1] Verify toggle_reaction reducer handles 6 allowed emojis in spacetimedb/src/index.ts (FR-D02)
- [x] T042 [D1] Create ReactionPicker component in src/components/Chat/ReactionPicker.tsx with 6 emojis (FR-D02)
- [x] T043 [D1] Display reaction groups with counts in src/components/Chat/MessageItem.tsx (FR-D04)
- [x] T044 [D1] Highlight user's own reactions in MessageItem.tsx (FR-D05)
- [x] T045 [D1] Respect ENABLE_EMOJI_REACTIONS flag in src/config/featureFlags.ts (FR-D06)
...
- [x] T046 [E1] Verify insert_system_message reducer in spacetimedb/src/index.ts creates records (FR-E01)
- [x] T047 [E1] Ensure clientConnected hook calls insert_system_message with 'connect' type in spacetimedb/src/index.ts (FR-E03)
- [x] T048 [E1] Ensure clientDisconnected hook calls insert_system_message with 'disconnect' type in spacetimedb/src/index.ts (FR-E04)
- [x] T049 [E1] Confirm system messages include channelId for filtering in spacetimedb/src/index.ts (FR-E05)
- [x] T050 [E1] Verify system_message subscription delivers in real-time in src/hooks/useChatMessages.ts (FR-E07)
...
- [x] T051 [E2] Style system messages with centered alignment in src/components/Chat/MessageItem.tsx (FR-E02, FR-032)
- [x] T052 [E2] Apply --color-text-muted to system messages in MessageItem.tsx (FR-032)
- [x] T053 [E2] Add smaller font size and badge style to system messages in MessageItem.tsx
- [x] T054 [E2] Group consecutive system messages with minimal spacing in MessageList.tsx
...
- [x] T055 [F1] Create useOnlineUsers hook in src/hooks/useOnlineUsers.ts filtering by online field (FR-F01, FR-F02)
- [x] T056 [F1] Filter offline users to show only last 7 days active in useOnlineUsers.ts (FR-F10)
- [x] T057 [F1] Display "Online" and "Offline" sections in src/pages/CommunityPage.tsx (FR-F03)
- [x] T058 [F1] Update user online status in clientConnected/Disconnected hooks in spacetimedb/src/index.ts (FR-F02, FR-F03)
...
- [x] T059 [F2] Create SessionWidget component in src/components/SessionWidget.tsx (FR-F05, FR-F06)
- [x] T060 [F2] Implement live "connected for Xm Ys" counter in SessionWidget.tsx (FR-F05)
- [x] T061 [F2] Display total session count for user in SessionWidget.tsx (FR-F06)
- [x] T062 [F2] Add graceful fallback for missing session data in SessionWidget.tsx
- [x] T063 [F2] Add SessionWidget to src/components/Header.tsx
...
- [x] T064 [G1] Audit src/ for hardcoded hex colors, replace with CSS custom properties from src/styles/theme.css (FR-001, SC-G05)
- [x] T065 [G1] Verify all spacing uses tokens (--space-N) not arbitrary px/rem in src/ (FR-002, SC-G06)
- [x] T066 [G1] Ensure Header.tsx height and styling consistent across pages (FR-029)
- [x] T067 [G1] Apply consistent hover states to all interactive elements in src/components/ (FR-025, FR-026)
...
- [x] T068 [G2] Verify --color-text-primary on --color-bg-base achieves ≥7:1 in src/styles/theme.css (FR-013)
- [x] T069 [G2] Ensure secondary text achieves ≥4.5:1 in theme.css (FR-014)
- [x] T070 [G2] Verify button text on --color-primary achieves ≥7:1 in theme.css (FR-013)
- [x] T071 [G2] Test focus indicators have ≥3:1 contrast in theme.css (FR-015)
...
- [x] T072 [G3] Verify heading sizes (--text-xl, --text-2xl) clearly larger than body in src/styles/theme.css (FR-021, FR-022)
- [x] T073 [G3] Apply font-weight-semibold to all headings in src/components/ (FR-022)
- [x] T074 [G3] Ensure primary actions use --color-primary while secondary use muted colors in src/components/ (FR-025)
- [x] T075 [G3] Add clear focus indicators (--color-border-focus) to all inputs in src/components/ (FR-028)
...
- [x] T076 [G4] Verify --color-bg-base uses pure black #0a0a0a in src/styles/theme.css (FR-010)
- [x] T077 [G4] Ensure green accent --color-primary has moderate saturation in theme.css (FR-009)
- [x] T078 [G4] Verify all transitions are 150-300ms in theme.css (FR-027, SC-G12)
- [x] T079 [G4] Apply font smoothing (-webkit-font-smoothing: antialiased) in theme.css (FR-024)
...
- [x] T080 [G5] Verify system message color is --color-text-muted in src/components/Chat/MessageItem.tsx (FR-032)
- [x] T081 [G5] Ensure system message font is --text-xs in MessageItem.tsx
- [x] T082 [G5] Confirm consecutive system messages group with minimal spacing in MessageList.tsx
...
- [x] T083 [P] [H1] Create StreamerProfilePage component in src/pages/StreamerProfilePage.tsx
- [x] T084 [P] [H1] Create useStreamerProfile hook in src/hooks/useStreamerProfile.ts using useTable(tables.streamerProfile)
- [x] T085 [H1] Display avatar with fallback to Rosie the Riveter in StreamerProfilePage.tsx (FR-H03)
- [x] T086 [H1] Display name and bio in StreamerProfilePage.tsx (FR-H01)
- [x] T087 [H1] Display social platform links with icons in StreamerProfilePage.tsx (FR-H02, FR-H01)
- [x] T088 [H1] Display online/offline colored dot indicator in StreamerProfilePage.tsx (FR-H04)
- [x] T089 [H1] Add route for /profile in src/App.tsx
...
- [x] T090 [P] [H2] Create update_streamer_profile reducer in spacetimedb/src/index.ts with ctx.sender === ADMIN_IDENTITY check (FR-H06)
...
- [x] T096 [P] [I1] Create populate_schedule reducer in spacetimedb/src/index.ts to seed 7 days: Stardew Valley, Farming Games, Fantasy Adventure, Science Fiction, Horror/Scary, Puzzle/Platformer, Any Category (FR-I02)
- [x] T097 [P] [I1] Create WeeklySchedule component in src/components/WeeklySchedule.tsx
- [x] T098 [I1] Fetch schedule using useTable(tables.streamScheduleDay) in WeeklySchedule.tsx (FR-I01)
- [x] T099 [I1] Display 7 days with themes (see T096) in WeeklySchedule.tsx (FR-I02)
- [x] T100 [I1] Highlight current day in WeeklySchedule.tsx (FR-I03)
- [x] T101 [I1] Add WeeklySchedule to StreamerProfilePage.tsx below bio (FR-I04)
...
- [x] T102 [P] [J1] Create VideoPlaceholder component in src/components/VideoPlaceholder.tsx (16:9 responsive)
- [x] T103 [P] [J1] Add 16:9 aspect ratio CSS to VideoPlaceholder.tsx (FR-J01, FR-J02)
- [x] T104 [J1] Update StreamPage in src/pages/StreamPage.tsx with VideoPlaceholder + chat layout (FR-J01)
- [x] T105 [J1] Implement live vs replay chat logic in StreamPage.tsx based on stream status (FR-J03, FR-J04)
- [x] T106 [J1] Filter chat messages by current stream's channel in StreamPage.tsx (FR-J04)
...
- [x] T107 [P] [J2] Create StreamStatusIndicator component in src/components/StreamStatusIndicator.tsx (FR-J05)
- [x] T108 [J2] Style indicator as colored dot (Discord/Slack/AIM style) in StreamStatusIndicator.tsx (FR-J05)
- [x] T109 [J2] Connect indicator to stream status from useStreamStatus hook in StreamStatusIndicator.tsx
- [x] T110 [J2] Add StreamStatusIndicator to StreamPage.tsx header (FR-J05)
...
- [x] T111 [P] [K1] Create report_message reducer in spacetimedb/src/index.ts (inserts with status='pending', reportedAt=ctx.timestamp) (FR-K02)
...
- [x] T118 [L1] Verify ChannelSidebar component exists in src/components/Chat/ChannelSidebar.tsx (FR-L01)
- [x] T119 [L1] Display channel list from useTable(tables.channel) in ChannelSidebar.tsx (FR-L01)
- [x] T120 [L1] Implement channel click handler to switch active channel in ChannelSidebar.tsx (FR-L02)
- [x] T121 [L1] Highlight active channel in sidebar in ChannelSidebar.tsx (FR-L03)
- [x] T122 [L1] Ensure CommunityPage.tsx includes ChannelSidebar
...
- [x] T123 [P] [L2] Create mark_channel_read reducer in spacetimedb/src/index.ts updating ChannelUnread table (FR-L06)
- [x] T124 [L2] Implement useChannelUnread hook to track unread per channel in src/hooks/useChannelUnread.ts (FR-L04)
- [x] T125 [L2] Display theme-matching dot next to unread channels in ChannelSidebar.tsx (FR-L04)
- [x] T126 [L2] Apply bold font-weight to unread channel names in ChannelSidebar.tsx (FR-L05)
- [x] T127 [L2] Call mark_channel_read when user switches to channel in ChannelSidebar.tsx (FR-L06)
- [x] T128 [L2] Confirm no browser notifications/sounds in MessageList.tsx (FR-L07)
...
- [x] T129 [P] Add 404 page in src/pages/NotFoundPage.tsx with chibi raven (eyes covered) (FR-H03)
...
- [x] T133 [P] Add prefers-reduced-motion support in src/styles/theme.css for accessibility
- [x] T134 [P] Add prefers-contrast support in theme.css for high contrast mode
